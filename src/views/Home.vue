...
<script>
    ...
    export default {
        ...
        methods: {
            setCity(){
                sessionStorage.setItem('myArea',JSON.stringify(this.$route.query.myArea));
            },
            //获取缩放比例
            getScale() {
            ...
            getJqtj() {
                this.$http({
                    method: 'post',
                    url: this.apiRoot + 'recJQTJB/findJQNumSJ',
                    headers: {
                    ...
                    data: {
                        ...
                        xzqhdm:this.$route.query.myArea
                    }
                })
                    .then(function (res) {
            ...
            getJqfl() {
                ...
                this.$http({
                    ...
                    if (res.status===200){
                        ...
                        let crr=[];
                        res.data.map(item=>{
                            nameArr.push(item.fldmmc);
                            valueArr.push(item.jjsl);
                            crr.push({
                                value: item['jjsl'],
                                symbolPosition: 'end'
                            });
                        });
                        this.jqflsjfxChart(nameArr, valueArr, crr);
                    }else{
                ...
                this.$http({
                    ...
                    if (res.status===200){
                        ...
                        if(res.data.jqflsyhb.ZTHB!=='{}'){
                            ...
                            code=res.data.jqflsyhb.ZTDM.slice(1,res.data.jqflsyhb.ZTDM.length-1);
                            function toJson(str) {
                            ...
                            code=toJson(code);
                            sessionStorage.setItem('type',JSON.stringify(code));
                            code = Object.keys(code).sort((a,b)=>{
                                return code[a]-code[b];
                            });
                            // console.log(c);
                            for (let item in code) {
                                // item=item.slice(0,item.length-2);
                                nameArr.push(code[item]);
                            }
                            for (let i=0;i<nameArr.length;i++){
                                ...
                                if (nameArr[i]+'环比' in hb){
                                    hbArr.push(hb[nameArr[i]+'环比'])
                                }else {
                                    // that.jqflhbfxSource[i].beforeYesterday='0';
                                    hbArr.push('0');
                                }
                        ...
                        // console.log(hb);
                        this.jqflhbfxChart(nameArr, yesterdayArr, beforeYesterdayArr, hbArr);
                    }else{
                        console.log(res.statusText);
                    }
                }).catch(res=>{
            ...
            getMapData() {
                ...
                this.$http({
                    method: 'post',
                    url: this.apiRoot + 'recJQTJB/findJQNumEverySJ',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                        'crossDomain': true
                    },
                    ...
                    .then(function (res) {
                        // console.log(res);
                        if(res.status){
                            that.ranking=res.data;
                            that.rankShow=true;
                            res.data.map(item => {
                                item.name = item.xzqhdm;
            ...
            getLianHb() {
                ...
                this.$http({
                    method: 'get',
                    url: this.apiRoot + 'processHandling/findEveryCityCaseNum',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                        'crossDomain': true
                    },
                    ...
                    /*this.shoulian.hb = parseFloat((res.data['Sjqhbfx']['有效警情环比'] * 100).toFixed(2));
                    // this.mapData.hb = parseFloat((res.data['Sjqhbfx']['有效警情环比'] * 100).toFixed(2));
                    if (this.shoulian.hb > 100) {
                        this.shoulian.hb = 100;
                    }
                    if (this.shoulian.hb < -100) {
                        this.shoulian.hb = -100
                    }*/
