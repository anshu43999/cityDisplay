			...
			<div class="l">
				<div class="l-t">
					<div class="chart-wrap">
						<!--                    标题-->
						<div class="title_wrap">警情统计监测</div>
						<!--                    内容-->
						<div class="chartBox">
							<!--                        数据-->
							<ul class="statistics">
	...
	export default {
		...
		data() {
			return {
				...
				jqtjjcSource: [
					{name: '处警数量占比', value: 0, radius: '50%'},
					{name: '有效警情占比', value: 0, radius: '50%'},
					{name: '反馈数量占比', value: 0, radius: '50%'}
				],
		...
		methods: {
			...
			mapChart() {
				...
				cityData.forEach(value => {
					data2.push({name: value.name, value: parseFloat(value.value / value.value1).toFixed(2), value1: value.value});
				});
				that.data1 = data2;
				//初显示   http://192.168.1.252:8082/dataDisplay/dictBJFSDMB/140000_full.json
				this.$http.get(this.apiRoot + 'dictBJFSDMB/getAll', {params: {json: '140001_full'}}).then(res => {
            ...
            // 跳转到立案数
            skiping(){
                this.$router.push({ path : '/index/handling' })
            },
	...
	main {
		...
		.r {
			...
			.r-t {
				width: 100%;
				height: 48%;
				background-image: url('../assets/images/index/r-t-bg.png');
				background-repeat: no-repeat;
				background-size: 100% 100%;
				position: relative;
			...
			.r-b {
				...
				height: 48%;
				position: relative;
				background-image: url('../assets/images/index/r-b-bg.png');
				background-repeat: no-repeat;
				background-size: 100% 100%;
